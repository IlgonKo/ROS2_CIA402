...
2. API Success/Fail 응답 계약과 exception 변환 규칙을 확정한다.
3. [Exception inventory](error_point_inventory.md)의 각 지점을 `API Fail`, `Alarm`, `Fault`,
   `Internal only`로 분류한다. 한 지점은 API Fail과 Diagnostic을 동시에 만들 수 있다.
4. 분류 결과를 바탕으로 TD-005의 exception 계층, API failure mapper, Diagnostic 관리,
