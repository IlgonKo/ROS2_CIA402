...
하지만 현재 `system/io/ethercat/param_catalog`는 주로 설정된 AP module의 ESI object를
반환하는 구조라서, 실제로 읽을 수 있는 station-level OD가 catalog와 IO Control Panel의
EC Parameter 목록에 충분히 표시되지 않는다.
...
→ 선택한 io device
→ 선택한 module/slot 또는 구성된 AP module 목록
→ module ESI object 목록 반환
```

station ESI object는 device profile에서 파싱되어 runtime/virtual OD 구성 등에 사용되지만,
catalog payload에서는 module object와 같은 방식으로 충분히 노출되지 않는다.
...
- sync/PDO assignment object가 catalog에 표시되지 않는다.
  ...
  - `0x1C12...0x1C13`
  - `0x1C32...0x1C33`
- 사용자는 process data가 정상인지, station diagnosis가 있는지, module diagnosis인지 구분하기 어렵다.
...
- 대신 사용자에게 필요한 구분은 `scope` 또는 `group`으로 제공한다.
...
2. 기존 module object 응답은 유지한다.
3. 각 object에 사용자가 구분할 수 있는 `scope` 또는 `group` 정보를 추가한다.
4. `pdo_mapping` 필드는 추가하지 않는다.
5. `ro p`는 `access: "ro"`로 표시한다.
6. IO Control Panel EC Parameter 탭에서 group별 표시 또는 필터링을 지원한다.
